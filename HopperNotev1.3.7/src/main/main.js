...
function writeData(payload) {
  const safePayload = {
    ...
    notebooks: Array.isArray(payload && payload.notebooks) ? payload.notebooks : []
  }
...
function buildPrintableEntryHtml(payload) {
  const title = String(payload && payload.title ? payload.title : "Untitled entry")
  const content = String(payload && payload.content ? payload.content : "")
  const tags = Array.isArray(payload && payload.tags) ? payload.tags.join(",") : ""
  const exportedAt = payload && payload.exportedAt ? new Date(payload.exportedAt) : new Date()
...
<body>
  <main data-hopper-export="entry" data-entry-tags="${tags}">
    ...
    <div class="entry-meta">Exported ${escapeHtml(exportedAt.toLocaleString())}</div>
...
ipcMain.handle("export-entry-pdf", async (event, payload) => {
  let pdfWindow

  try {
    const title = String(payload && payload.title ? payload.title : "journal-entry")
    const safeName = title.replace(/[<>:"/\|?* -]/g, "").trim() || "journal-entry"
    ...
    await pdfWindow.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(buildPrintableEntryHtml(payload))}`)
    ...
    return { ok: false, error: e.message || "Failed to export PDF" }
  } finally {
