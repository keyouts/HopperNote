...
const ALLOWED_EDITOR_CLASSES = new Set([
  ...
  "hopper-highlight-title", "hopper-highlight-text", "hopper-highlight-note", "hopper-highlight-url", "inline-tag", "md-table",
...
function normalizeEntry(raw) {
  ...
  return {
    ...
    content,
    tags: mergeTags(Array.isArray(raw && raw.tags) ? raw.tags.slice(0, LIMITS.tags).map(normalizeTag).filter(Boolean) : [], extractTagsFromText(htmlToPlainText(content))),
    createdAt,
    updatedAt,
...
function normalizeHighlight(raw) {
  ...
  return {
    ...
    note,
    tags: getHighlightTags({ text, note }).slice(0, LIMITS.tags),
...
function dedupeHighlights(highlights) {
  ...
  return Array.from(byKey.values()).sort((a, b) => b.timestamp - a.timestamp).slice(0, LIMITS.highlights)
}
...
function loadLocalState() {
  try {
    ...
    if (!raw || validation.byteLength(raw) > LIMITS.localStateBytes) return { entries: [], highlights: [], notebooks: [] }
    ...
    copyBtn.addEventListener("click", async ev => {
      ...
      try {
        if (window.api && typeof window.api.copyText === "function") {
          await window.api.copyText(textToCopy)
...
function applyInsertedHighlightColors(root = editor) {
  ...
  root.querySelectorAll(".hopper-highlight-card").forEach(card => {
    card.dataset.color = getSafeHighlightColor(card.dataset.color || card.getAttribute("data-color"))
...
function parseCsv(text) {
  ...
  if (validation.byteLength(input) > LIMITS.importBytes) throw new Error("CSV file exceeds the allowed size")
  ...
  for (let index = 0; index < input.length; index++) {
    ...
    if ((character === "\n" || character === "\r") && !quoted) {
      ...
      if (records.length > LIMITS.highlights + 1) break
      record = []
      cell = ""
      continue
    }
  ...
  record.push(cell)
  if (record.some(value => value !== "") && records.length <= LIMITS.highlights) records.push(record)
  if (!records.length) return []

  const headers = records[0].map(header => header.trim().toLowerCase())
  return records.slice(1, LIMITS.highlights + 1).map(values => {
    const row = {}
...
function serializeCsv(rows) {
  ...
  return [
    headers.join(","),
    ...rows.slice(0, LIMITS.highlights).map(row => headers.map(key => escapeCell(row[key] || "")).join(","))
  ].join("\n")
}
...
function importHighlightsFromCsvText(text) {
  try {
    ...
    renderTags()
    queueSave()
    announce(`${imported.length} highlights imported`)
  } catch (error) {
    announce(error.message || "CSV import failed")
  }
}
...
function handleImportCsv() {
  ...
  input.addEventListener("change", () => {
    ...
    if (file.size <= 0 || file.size > LIMITS.importBytes) {
      announce("CSV file exceeds the allowed size")
      return
    }

    const reader = new FileReader()
    reader.onload = () => importHighlightsFromCsvText(String(reader.result || ""))
    reader.onerror = () => announce("CSV import failed")
