...
test("restricts colors", () => {
  ...
  assert.equal(validation.safeColor("#c8ff00"), "#c8ff00")
  assert.equal(validation.safeColor("rgb(240, 200, 10)"), "rgb(240, 200, 10)")
...
test("preserves highlight metadata", () => {
  const state = validState()
  state.highlights[0].color = "#abcdef"
  state.highlights[0].colorName = "Custom"
  state.highlights[0].pageTitle = "Page"
  state.highlights[0].ranges = [{ exact: "Text", prefix: "Before", suffix: "After" }]
  state.highlights[0].pinned = true
  const highlight = validation.normalizeState(state).highlights[0]
  assert.equal(highlight.color, "#abcdef")
  assert.equal(highlight.colorName, "Custom")
  assert.equal(highlight.pageTitle, "Page")
  assert.equal(highlight.ranges.length, 1)
  assert.equal(highlight.pinned, true)
})
