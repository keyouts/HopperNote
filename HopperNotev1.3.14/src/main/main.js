...
const { APP_SCHEME, APP_URL, resolveAppRequestPath } = require("./app-protocol")
...
function registerAppProtocol() {
  appSession.protocol.handle(APP_SCHEME, request => {
    const filePath = resolveAppRequestPath(app.getAppPath(), request.url)
...
function reportStartupFailure(message) {
  ...
  dialog.showErrorBox("Hopper Note could not start", `${message}\n\nPlease reinstall Hopper Note. If the problem continues, keep this message for troubleshooting.`)
}
...
async function createWindow() {
  ...
  mainWindow.webContents.on("preload-error", (event, preloadPath, error) => {
    reportStartupFailure(`The startup bridge failed to load: ${error && error.message ? error.message : "unknown preload error"}`)
  })
  mainWindow.webContents.on("did-fail-load", (event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    if (!isMainFrame) return
    reportStartupFailure(`The application interface failed to load (${errorCode}): ${errorDescription || validatedURL || "unknown load error"}`)
  })
  ...
  try {
    ...
    reportStartupFailure(`The application interface failed to load: ${error && error.message ? error.message : String(error)}`)
    if (mainWindow && !mainWindow.isDestroyed()) mainWindow.show()
  }
}
