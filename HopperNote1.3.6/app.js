...
function syncCurrentEntryFromEditor() {
  ...
  try {
    ...
    const rawContent = (typeof markdownMode !== "undefined" && markdownMode) ? markdownToHtml(editor.textContent || "") : editor.innerHTML
...
function shouldSkipTagNode(node) {
  ...
  return !!parent.closest(".inline-tag, .wiki-link, .hopper-highlight-card, script, style")
}
...
function htmlToMarkdown(html) {
  const div = document.createElement("div")
  div.innerHTML = removeWikiLinkRendering(html || "")
...
function markdownToHtml(markdown) {
  ...
  lines.forEach(line => {
    ...
    if (/^#{1,6}\s+/.test(trimmed)) {
      html += `<h2>${escapeHtml(trimmed.replace(/^#{1,6}\s+/, ""))}</h2>`
...
function setMarkdownMode(nextMode) {
  ...
  if (entry && wantsMarkdown) {
    const htmlSnapshot = sanitizeEditorHtml(removeWikiLinkRendering(editor.innerHTML || entry.content || "<p><br></p>"))
    markdownSession = {
      entryId: entry.id,
      htmlSnapshot,
    ...
    const currentMarkdown = editor.textContent || ""
    if (markdownSession && markdownSession.entryId === entry.id && currentMarkdown === markdownSession.markdownSnapshot) {
  ...
  if (markdownToggleBtn) {
    markdownToggleBtn.textContent = markdownMode ? "Rich Text View" : "Markdown View"
...
function snapshotEntry() {
  ...
  const current = sanitizeEditorHtml((typeof markdownMode !== "undefined" && markdownMode) ? markdownToHtml(editor.textContent || "") : removeWikiLinkRendering(editor.innerHTML))
...
function getEntryTitleMap(entries = state.entries) {
  const map = new Map()
  ;(entries || []).forEach(entry => {
    const key = normalizeWikiTitle(entry && entry.title)
    if (key && !map.has(key)) map.set(key, entry)
  })
  return map
}

function getCurrentEditorContentForLinks(entry) {
  if (!entry || entry.id !== selectedEntryId || !editor) return entry && entry.content ? entry.content : ""
  if (typeof markdownMode !== "undefined" && markdownMode) return markdownToHtml(editor.textContent || "")
  return removeWikiLinkRendering(editor.innerHTML || "")
}

function extractWikiLinkTitlesFromText(text) {
  const titles = []
  const seen = new Set()
  const re = /\[\[([^\]]+)\]\]/g
  let match
...
function extractWikiLinkTitlesFromHtml(html) {
  const cleaned = removeWikiLinkRendering(html || "")
  const titles = extractWikiLinkTitlesFromText(cleaned)
  const plainTitles = extractWikiLinkTitlesFromText(htmlToPlainText(cleaned))
  return Array.from(new Map(titles.concat(plainTitles).map(title => [normalizeWikiTitle(title), title])).values())
}

function extractWikiLinkTitlesFromEntry(entry) {
  if (!entry) return []
  return extractWikiLinkTitlesFromHtml(getCurrentEditorContentForLinks(entry))
}

function removeWikiLinkRendering(html) {
  const div = document.createElement("div")
  div.innerHTML = html || ""
  div.querySelectorAll(".wiki-link").forEach(node => {
    const title = node.dataset.title || (node.textContent || "").replace(/^\[\[|\]\]$/g, "")
    node.replaceWith(document.createTextNode(`[[${title}]]`))
  })
  return div.innerHTML
}

function makeEntryLink(sourceEntryId, title, index, titleMap) {
  const normalizedTitle = normalizeWikiTitle(title)
  const target = titleMap.get(normalizedTitle) || null
  return {
    id: `${sourceEntryId}:${normalizedTitle}:${index}`,
    sourceEntryId,
    targetEntryId: target ? target.id : "",
    title,
    normalizedTitle,
    resolved: !!target
  }
...
function buildLinksForEntry(entry, titleMap) {
  if (!entry) return []
  return extractWikiLinkTitlesFromEntry(entry).map((title, index) => makeEntryLink(entry.id, title, index, titleMap))
}

function rebuildLinkIndexForEntries(entries) {
  const list = Array.isArray(entries) ? entries : []
  const titleMap = getEntryTitleMap(list)
  const links = []
  const nextEntries = list.map(entry => {
    const entryLinks = buildLinksForEntry(entry, titleMap)
    links.push(...entryLinks)
    return { ...entry, links: entryLinks }
  })
  return { entries: nextEntries, links }
}

function rebuildLinkIndex() {
  const titleMap = getEntryTitleMap()
  const links = []

  state.entries.forEach(entry => {
    entry.links = buildLinksForEntry(entry, titleMap)
...
function updateLinksForEntry(entry) {
  if (!entry) return []
  const titleMap = getEntryTitleMap()
  entry.links = buildLinksForEntry(entry, titleMap)
  state.links = (state.links || []).filter(link => link.sourceEntryId !== entry.id).concat(entry.links)
  return entry.links
}

function getOutgoingLinks(entry) {
  if (!entry) return []
  return entry.links || []
}

function getIncomingLinks(entry) {
  if (!entry) return []
  const currentTitle = normalizeWikiTitle(entry.title)
  return (state.links || [])
    .filter(link => link.sourceEntryId !== entry.id && (link.targetEntryId === entry.id || (!link.targetEntryId && link.normalizedTitle === currentTitle)))
    .map(link => ({ link, entry: getEntryById(link.sourceEntryId) }))
...
function renderWikiLinksInHtml(html) {
  ...
  nodes.forEach(node => {
    ...
    text.replace(/\[\[([^\]]+)\]\]/g, (full, title, index) => {
      frag.appendChild(document.createTextNode(text.slice(last, index)))
      const cleanTitle = String(title || "").replace(/\|.*$/, "").trim()
      const linkedEntry = titleMap.get(normalizeWikiTitle(cleanTitle))
      const button = document.createElement("button")
      button.type = "button"
      button.className = "wiki-link" + (linkedEntry ? "" : " missing")
      button.dataset.title = cleanTitle
      button.setAttribute("contenteditable", "false")
      button.setAttribute("aria-label", linkedEntry ? `Open linked entry ${cleanTitle}` : `Create linked entry ${cleanTitle}`)
      if (linkedEntry) button.dataset.entryId = linkedEntry.id
      button.textContent = `[[${cleanTitle}]]`
...
function openWikiTitle(title) {
  ...
  const existing = getEntryTitleMap().get(normalizeWikiTitle(cleanTitle))
  if (existing) {
    selectEntry(existing.id)
    announce(`Opened ${existing.title}`)
    return
  }

  const confirmed = window.confirm(`Create a new entry named "${cleanTitle}"?`)
  if (!confirmed) return
...
function renderLinkPanel() {
  ...
  if (!current) {
    linkPanel.textContent = "[[Links]] and Backlinks appear here."
    return
  }

  const outgoing = getOutgoingLinks(current)
  const incoming = getIncomingLinks(current)
  const section = document.createElement("div")
  section.className = "link-panel-grid"

  const makeGroup = (label, items, renderer) => {
    const group = document.createElement("div")
    group.className = "link-panel-group"
    ...
    items.forEach(item => group.appendChild(renderer(item)))
    return group
  }
  ...
  const renderOutgoing = item => {
    const btn = document.createElement("button")
    btn.type = "button"
    btn.className = "link-chip" + (item.resolved ? "" : " missing")
    btn.textContent = item.resolved ? (getEntryById(item.targetEntryId)?.title || item.title) : `${item.title} +`
    btn.title = item.resolved ? "Open linked entry" : "Create linked entry"
    btn.addEventListener("click", () => openWikiTitle(item.title))
    return btn
  }
  ...
  const renderIncoming = item => {
    ...
    btn.textContent = item.entry.title || "Untitled entry"
    btn.title = `Linked as [[${item.link.title}]]`
    btn.addEventListener("click", () => selectEntry(item.entry.id))
    return btn
  }

  section.appendChild(makeGroup("Links", outgoing, renderOutgoing))
  section.appendChild(makeGroup("Backlinks", incoming, renderIncoming))
  linkPanel.appendChild(section)
}

if (editor) {
  editor.addEventListener("click", ev => {
    const link = ev.target.closest && ev.target.closest(".wiki-link")
    if (!link) return
    ev.preventDefault()
    openWikiTitle(link.dataset.title || link.textContent.replace(/^\[\[|\]\]$/g, ""))
  })
}
